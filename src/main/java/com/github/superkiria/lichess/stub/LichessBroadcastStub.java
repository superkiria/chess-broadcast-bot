...
public class LichessBroadcastStub {
    ...
    @GetMapping(value = "/stream/broadcast/round/qwerty.png", produces = MediaType.APPLICATION_STREAM_JSON_VALUE)
    ...
    public LichessEvent timetable() {
        return LichessEvent.builder()
                ...
                .rounds(Collections.singletonList(LichessRound.builder().ongoing(true).name("test LichessRound").id("qwerty").finished(false).slug("").startsAt(null).url("").build())).build();
    }
